// ============================================================================
//  ARIE Finance — Management Report Builder API  (Vercel serverless)
//  Route:  /api/managementreport
//  Purpose: AUTHENTICATION / SESSION ONLY for the stateless tool at
//           /managementreport.
// ----------------------------------------------------------------------------
//  This is a deliberate STATELESS sibling of /api/salestracker. It reuses the
//  Sales Tracker's proven password -> HMAC-session-cookie pattern, but it
//  stores NO report data of any kind. There is no board, no history, no draft,
//  no snapshot, no report configuration on the server. The only server-side
//  state is session infrastructure:
//
//  Redis keys (session infrastructure ONLY — never report content):
//    mr:signkey          STR  server HMAC key for session tokens
//    mr:loginfail:<ip>   STR  failed-login throttle counter (TTL)
//
//  Confidential report data (financials, commentary, client data, uploaded
//  workbooks, generated PDF/PPTX, continuation .data files) lives ONLY in the
//  operator's browser memory and in the files they download. None of it is
//  ever sent to or stored by this API.
// ----------------------------------------------------------------------------
//  ENV (production Vercel project):
//    MANAGEMENTREPORT_PASSCODE               staff passcode (secret; never in source)
...
//  no report content is written under them.
// ----------------------------------------------------------------------------
//  Actions:
...
const PASSCODE = process.env.MANAGEMENTREPORT_PASSCODE || '';
...
async function signKey() {
  ...
  let k = await redis(['GET', 'mr:signkey']);
  if (!k) { const fresh = crypto.randomBytes(32).toString('hex'); await redis(['SET', 'mr:signkey', fresh, 'NX']); k = await redis(['GET', 'mr:signkey']); }
  _signKey = k; return k;
}
...
function setCookie(res, req, token, maxAgeS) {
  const parts = [COOKIE + '=' + token, 'Path=/api/managementreport', 'Max-Age=' + maxAgeS, 'HttpOnly', 'SameSite=Strict'];
  if (cookieSecure(req)) parts.push('Secure');
  res.setHeader('Set-Cookie', parts.join('; '));
}
function sessionToken(req) { const c = req.headers.cookie || ''; const m = c.match(/(?:^|;\s*)arie_mr=([^;]+)/); return m ? decodeURIComponent(m[1]) : ''; }
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'login') {
      ...
      const failKey = 'mr:loginfail:' + crypto.createHash('sha256').update(ip).digest('hex').slice(0, 24);
    ...
    // forced to sign in again. No report data is ever read or written here.
